...
use nstable_exchange::{ContractContract as Exchange, PoolInfo, ContractMetadata};
...
pub struct NStableStorageState {
    pub deposit: U128,
    pub usage: U128,
}
...
pub fn get_storage_state(
    pool: &ContractAccount<Exchange>, 
    account_id: ValidAccountId
) -> Option<NStableStorageState> {
    let sb = view!(pool.get_user_storage_state(account_id)).unwrap_json_value();
    if let Value::Null = sb {
        None
    } else {
        let ret: NStableStorageState = from_value(sb).unwrap();
        Some(ret)
    }
}
