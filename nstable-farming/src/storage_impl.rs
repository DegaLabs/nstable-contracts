...
impl StorageManagement for Contract {
    ...
    fn storage_deposit(
        ...
        let already_registered = self.data().stakers.contains_key(&account_id);
        ...
        if already_registered {
            ...
            self.data_mut().stakers.insert(&account_id, &VersionedStaker::new(account_id.clone()));
            self.data_mut().staker_count += 1;
    ...
    fn storage_unregister(&mut self, force: Option<bool>) -> bool {
        ...
        // User should withdraw all his rewards and locktokens token before unregister!

        let account_id = env::predecessor_account_id();
        if let Some(staker) = self.get_staker_wrapped(&account_id) {
            
            assert!(
                staker.get_ref().rewards.is_empty(),
                "{}", ERR12_STORAGE_UNREGISTER_REWARDS_NOT_EMPTY
            );
            assert!(
                staker.get_ref().locktoken_powers.is_empty(),
                "{}", ERR13_STORAGE_UNREGISTER_LOCKTOKEN_POWER_NOT_EMPTY
            );
            // todo: how about his rps lookup map? maybe already cleaned when unstake all locktokens
            self.data_mut().stakers.remove(&account_id);
            self.data_mut().staker_count -= 1;
    ...
    fn storage_balance_of(&self, account_id: ValidAccountId) -> Option<StorageBalance> {
        if self.data().stakers.contains_key(&account_id.into()) {
            Some(StorageBalance {
